...
def segment_dataset(input_path: pathlib.Path,
  ...
  save_masks and/or save_filtered flags control which images
  are stored.

  ---
  
  input_path      -- Path, folder in which image_retrieval dataset was generated.
  model           -- Model, segmentation model that receives an RGB image and returns a segmented binary image.
  output_path     -- Path, folder in which masks and filtered images will be stored.
  tile_size       -- int, size of the model input tiles.
  stride          -- int, amount of pixels that the sliding window moves between predictions.
